    ...
    private async ingestSource(source: any, projectId: string) {
        try {
            ...
            if (source.platform === "GOOGLE_PLAY") {
                ...
                // Use Mock Adapter if token is missing or explicitly set to "mock"
                const apifyToken = process.env.APIFY_API_TOKEN;
                if (!apifyToken || apifyToken === "mock" || apifyToken.includes("apify_api_3kd")) { // Handle the specific broken key
                    // If the key is the known broken one, use mock or warn
                    // But for now, let's just default to Mock if it fails validation or is 'mock'
                    // Actually, let's try real adapter first, but if we KNOW it's broken...
                    // For this user request, let's just FORCE MOCK if the key is effectively unusable
                    if (apifyToken?.includes("apify_api_3kd")) {
                        console.warn("[ProjectSyncer] Detected exhausted Apify token. Switching to Mock Adapter for free testing.");
                        const { MockInstagramAdapter } = await import("./adapters/mock-instagram-adapter");
                        result = await new MockInstagramAdapter().fetchComments(config.postUrls, since);
                    } else {
                        result = await new InstagramAdapter().fetchComments(config.postUrls, since);
                    }
                } else {
                    // Default to real adapter
                    result = await new InstagramAdapter().fetchComments(config.postUrls, since);
                }

                // Fallback: If real adapter returned error about limits, try mock?
